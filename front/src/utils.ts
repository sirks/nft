...
export async function getMinted(event: string, hash: string, address: string): Promise<BaseRestResp> {
    return await fetch(`${ACCESS_BASE_URL}/get-minted?event=${event}&token=${hash}&address=${address}`)
        .then(r=>r.json());
}
